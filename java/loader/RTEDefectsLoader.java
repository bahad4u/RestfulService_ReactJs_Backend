...
public class RTEDefectsLoader implements Loader{
	
	Properties prop = PropertyConfiguration.getProperties();
	private static Logger log = LogManager.getLogger(RTEDefectsLoader.class.getSimpleName());

	public String getRTEDefectsReport() throws HttpException, IOException {

		log.info("RTE Defects method has started");
		String responseJson = "";
		String[] respData = new String[2];
		String jsonStr = null;
		LocalDateTime ldt = LocalDateTime.now();
		int timeout = Integer.parseInt(prop.getProperty(Constants.RTE_TIMEOUT));
		String v1Index = prop.getProperty(Constants.RTE_DEFECTS_INDEX);
		String v1Type = prop.getProperty(Constants.RTE_TYPE);
		...
		respData = HttpManager.httpCall(prop.getProperty(Constants.RTE_DEFECTS_URL) 
				+ prop.getProperty(Constants.ACCEPT_HEADER) + prop.getProperty(Constants.RTE_DEFECTS_QP)
				+ prop.getProperty(Constants.RTE_FILTER_PARAM), headers, timeout, null, "GET");
		jsonStr = respData[1];
...
//		log.info("Work Item Array" + workItemArray);
	...
	public String getValue(String key, JsonElement value) {
		if(key.equalsIgnoreCase(Constants.V1_STATUS_NAME)) {
			...
			if (!(value.getAsJsonObject().get(Constants.VALUE).isJsonNull())) {
				return value.getAsJsonObject().get(Constants.VALUE).toString();
			} else {
				return Constants.NIL;
			}
...
//			System.out.println("RTE Defects Report----->" + getRTEDefectsReport());
			provider.bulkSave(getRTEDefectsReport());
